...
namespace GetTogethers
{
    ...
    //TODO ... come on now I know I can do much better
        ...
        static IEnumerable<CodeInstruction> Transpiler(ILGenerator generator, IEnumerable<CodeInstruction> instructions)
        {
            ...
            for(int i = 0; i < codes.Count; i++) {
                ...
                if ((i + 7) < codes.Count && codes[i + 7].opcode == OpCodes.Call && codes[i + 7].operand == listConcat) {
                    ...
                    yield return codes[i + 7];		//Pawn, IEnumerable<ICommuncation> on stack
                    yield return new CodeInstruction (OpCodes.Ldloc_S, 8);	//Pawn, IEnumerable<ICommuncable>, List<FloatMenuOption> on stack
