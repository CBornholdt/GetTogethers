...
namespace GetTogethers
{
    public class DynamicCommManager : MapComponent
    {
        ...
        {
            return comms.Where(commStruct => commStruct.communication == comm)
                                .Take(1).ForEachWCount(this.RemoveCommStruct) > 0;
        }

        public int RemoveAllCommunicationsOfType<T>() where T : Communication
        {
            return comms.Where(commStruct => commStruct.communication is T)
                                .ForEachWCount(this.RemoveCommStruct);
        }
                ...
                if (comm.enabled && comm.ticksUntilDisabled > 0) {
                    if (--(comm.ticksUntilDisabled) == 0) {
                        ...
                        if(comm.removeWhenDisabled)
                            this.RemoveCommStruct(comm);
                    }
                }
            }
        }

        private void RemoveCommStruct(CommStruct commStruct)
        {
            commStruct.communication.TryRemoveNotificationLetter();
            this.comms.Remove(commStruct);
        }
